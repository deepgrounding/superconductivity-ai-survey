...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Figure 1: The taxonomy, with its occupancy. Every corpus record carries one label from Axis 1 and one from Axis 2; the AI lens applies only to records that use a machine-learning method in their own work. Counts are seed-corpus papers per label on the same basis as Table 1, so the two agree; the AI-lens counts are over the whole working corpus.}]{figures/fig1_taxonomy.png}}
\caption*{Figure 1: The taxonomy, with its occupancy. Every corpus record
...
Counts are seed-corpus papers per label on the same basis as Table 1, so
the two agree; the AI-lens counts are over the whole working corpus.}
\end{figure}
