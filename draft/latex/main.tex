...
\urlstyle{same}
\hypersetup{
  pdftitle={AI for Superconductivity: A Systematic Map and Readiness Assessment},
  pdfauthor={Mingguang Chen1,*, Bo Qu1; 1DeepGrounding. *Corresponding author: deepgroundingai@gmail.com},
  colorlinks=true,
  linkcolor={Maroon},
  filecolor={Maroon},
  citecolor={Blue},
...
\title{AI for Superconductivity: A Systematic Map and Readiness
Assessment}
\author{Mingguang Chen\textsuperscript{1,$*$}, Bo Qu\textsuperscript{1}\\[6pt]
...
obvious target for AI for Science and a poorly understood one. Every
arXiv preprint filed under cond-mat.supr-con between September 2021 and
September 2026 was harvested month by month, giving 10,303 records, of
which 10,049 survive off-topic filtering at a measured bleed rate of
2.5\%; a targeted supplement adds 200 records. Each is classified along
two axes -- material family and mode of inquiry -- by a language-model
pass agreeing with hand labels at 86.3\% and 87.0\%, per-class recall
reported. We then assess 15 of the grid's 45 cells for data
availability, benchmark maturity and validation cost. Machine learning
has barely entered the field: 1.2\% of the seed uses an AI method in its
own work. Where it has entered it clusters in ab initio screening and
neural quantum states, where a calculation can check a prediction, and
in device metrology, where instruments label their own output; we offer
validation cost as the hypothesis fitting that pattern, untested against
data volume or scientific stakes. Synthesis, the rate-limiting step
across families, carries no AI-tagged paper in the seed and one in the
merged corpus. The gap between prediction and measurement is concrete:
recent screens advertise candidates above 200 K at 200 GPa, while the
closed prediction-synthesis-measurement loops found here delivered
low-temperature intermetallics, 2.5 to 6.5 K at ambient pressure in the
more recent. We conclude that the highest-value interventions are
releasing labelled characterization data, building a time-stamped
benchmark of experimentally confirmed superconductors, and modelling
synthesizability rather than automating synthesis. Corpus, labels and
...
it studies and the mode of inquiry it uses -- and then evaluate the
cells of the resulting grid for what a machine-learning method could
contribute to them. The corpus is 10,249 records after off-topic
...
Three results follow. Machine learning has barely entered this field:
1.2\% of the systematically harvested corpus uses an AI method in its
own work. Where it has entered, it tracks validation cost, clustering in
the cells where a calculation can check a prediction and in device
metrology where instruments generate their own labels, rather than in
the cells with the most data or the greatest scientific stakes. And the
field's actual bottleneck is the emptiest cell: synthesis, which carries
no AI-tagged paper in the systematic seed at all, and one in the merged
corpus.
...
gives the readiness assessment for 15 selected cells and the resulting
priorities. Section 7 asks what the emerging AI-for-science
infrastructure would have to deliver for the field's central target to
come within reach, and what would falsify that route. Section 8 offers
three testable hypotheses about why the field looks the way it does.
...
\emph{Critical temperature.} \(T_c\) is quoted from several inequivalent
operational definitions: the onset of a resistive drop, the midpoint of
the transition, the zero-resistance point, and the temperature at which
diamagnetic screening appears. The gap between onset and zero resistance
is small in clean conventional samples and large in inhomogeneous ones,
which is exactly where contested claims live. Throughout this survey we
say which definition a reported number uses when the distinction bears
on the claim, and we treat a resistive onset without a magnetic
signature as weaker evidence than the two together.

\emph{Conventional and unconventional.} We use \emph{conventional} to
...
\emph{Prediction versus discovery.} A computed structure with a
...
\emph{AI-discovered.} We treat a material as AI-discovered only when a
...
\emph{Axis 1, material family}, is what the paper is about:
...
\emph{Axis 2, mode of inquiry}, is how the work was done:
...
\emph{The AI lens} applies only to papers that use a machine-learning
...
are organised along them. Section 6 evaluates 15 selected cells for what
AI can currently do in them.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Figure 1: The taxonomy and its occupancy. Every corpus record carries one label from Axis 1 and one from Axis 2; the AI lens applies only to records that use a machine-learning method in their own work. Labels are sorted by count within each block and bars are scaled within a block, since the three blocks are different populations. Family and mode counts are seed-corpus papers on the same basis as Table 1, so the two agree; the AI-lens counts are over the whole working corpus. Two features carry into later sections: theory alone is 51.2\% of the corpus, and the entire AI lens is 253 papers, of which one is an autonomous experiment.}]{figures/fig1_taxonomy.png}}
\caption*{Figure 1: The taxonomy and its occupancy. Every corpus record
carries one label from Axis 1 and one from Axis 2; the AI lens applies
only to records that use a machine-learning method in their own work.
Labels are sorted by count within each block and bars are scaled within
a block, since the three blocks are different populations. Family and
mode counts are seed-corpus papers on the same basis as Table 1, so the
two agree; the AI-lens counts are over the whole working corpus. Two
features carry into later sections: theory alone is 51.2\% of the
corpus, and the entire AI lens is 253 papers, of which one is an
...
harvested corpus, and then asks of the cells it scores what would have
to be true for a machine-learning method to contribute there. The result
is a statement about where the opportunities are \emph{not}, which the
...
\emph{Seed harvest.} Every arXiv preprint listed under
...
\emph{Targeted supplement.} The seed necessarily misses AI-for-materials
work that never touches the superconductivity category. Two
supplementary threads were harvested by keyword: AI applied to
...
\texttt{supplement\_t1} and \texttt{supplement\_t2}. The supplement is
recency-biased by construction -- its queries are capped and sorted by
submission date -- so it is excluded from every growth statistic and
from Table 1's family-by-mode counts, where it appears only as a
separate column.

\emph{Off-topic filtering.} 745 records were flagged as not about
superconductivity and excluded from all statistics. Within the
systematically harvested seed the rate is 254 of 10,303, or 2.5\%, which
is this corpus's measured query-bleed. The rate is far higher in the
supplement, as intended: the general AI-for-materials thread was
designed to reach outside the field. After exclusion the working corpus
is 10,249 records: 10,049 seed and 200 supplement.

\emph{Labelling.} Both axes and the AI lens were assigned by a
language-model pass over title and abstract
(\texttt{gemini-3.5-flash-lite}), after keyword rules were measured and
found inadequate. The hand-labelled sample is 150 stratified records, of
which four were hand-marked as query bleed and carry no mode, so both
agreement figures below are scored on the remaining 146 -- 126 of 146 on
family, 127 of 146 on mode:
...
Per-class recall is 100\% (theory, 57/57), 90\% (characterization,
47/52), 85\% (ab initio, 11/13), 64\% (synthesis, 7/11) and 38\%
(discovery, 5/13). The two small classes are the weak ones, and the
denominators are the reason to be careful with them: on thirteen papers
a single reclassification moves recall by eight points, so these
percentages carry roughly the information of the counts beside them. One
claim in this survey does rest on them, and it should be read with that
in mind: the near-absence of AI-tagged synthesis work depends on both
synthesis recall (64\%) and AI recall (77\%), and both errors run in the
direction of undercounting. The AI lens was validated separately against
120 hand-labelled records, half drawn from each side of the rule-based
tag: 88\% precision and 77\% recall on the AI-versus-not decision, 82\%
exact agreement on which method.

\emph{These figures score the labels that ship with this paper}, not a
fresh model run. \texttt{llm\_label.py\ -\/-score-published} reads the
released label cache and the released truth set and reproduces every
number above; the \texttt{-\/-benchmark} mode re-queries the model and
is used only for choosing between models, since its result depends on a
run the reader cannot repeat.

\emph{Taxonomy revision, disclosed.} The two agreement figures above are
not measured against the same label set. The task axis originally
...
\emph{Annotator disclosure.} All hand labels were produced by a single
...
\emph{Citation counts.} Citation data comes from OpenAlex, resolved for
8,280 of the records. Over a five-year window raw counts favour older
work mechanically, so where this survey ranks papers it ranks by
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Figure 2: Semantic map of the seed corpus (TF-IDF, truncated SVD, t-SNE), coloured by material family. Each family is labelled at its density peak, with a leader line where the label had to be moved to avoid a collision; nine categorical hues cannot be told apart reliably, so the labels rather than the colours carry identity. Per-family counts match Table 1.}]{figures/fig2_semantic_map.png}}
\caption*{Figure 2: Semantic map of the seed corpus (TF-IDF, truncated
SVD, t-SNE), coloured by material family. Each family is labelled at its
density peak, with a leader line where the label had to be moved to
avoid a collision; nine categorical hues cannot be told apart reliably,
so the labels rather than the colours carry identity. Per-family counts
match Table 1.}
\end{figure}

\emph{Limitations.} The corpus is arXiv-only, so it inherits arXiv's
...
\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Figure 3: Quarterly submissions, seed records only, drawn as small multiples rather than as one nine-line overlay. Panels are ordered by growth, measured as the change from the first four to the last four complete quarters, and that figure is printed on each panel. The nine family panels (a) share one vertical scale so magnitudes stay comparable; the five mode panels (b) use independent scales, each with its maximum printed, because a shared scale renders the three smaller modes as flat lines. Both partial endpoint quarters are omitted. The marked cuprate spike in 2023Q3 is the LK-99 replication burst discussed in the text.}]{figures/fig3_growth_timeline.png}}
\caption*{Figure 3: Quarterly submissions, seed records only, drawn as
small multiples rather than as one nine-line overlay. Panels are ordered
by growth, measured as the change from the first four to the last four
complete quarters, and that figure is printed on each panel. The nine
family panels (a) share one vertical scale so magnitudes stay
comparable; the five mode panels (b) use independent scales, each with
its maximum printed, because a shared scale renders the three smaller
modes as flat lines. Both partial endpoint quarters are omitted. The
marked cuprate spike in 2023Q3 is the LK-99 replication burst discussed
in the text.}
\end{figure}
...
Nickelates are among the smaller named families in this corpus (628 seed
records) and by a wide margin the fastest-growing: quarterly output rose
203\% between the first and last four quarters of the window, against
+55\% for the next fastest (Figure 3a). The line begins outside the
window with superconductivity in an infinite-layer nickelate thin film
\citep{li2019superconductivity}, and turns sharply with the report of
superconductivity near 80 K in La\(_3\)Ni\(_2\)O\(_7\) above 14 GPa
\citep{sun2023superconductivity}.
...
For AI purposes the nickelate story is instructive in a discouraging
way: the rate-limiting steps were sample quality, oxygen stoichiometry
and pressure technique. None of them is a prediction problem.
...
(1,487 seed records) and the only one where computational prediction
...
which is defensible chemically and wrong physically. It is worth
quantifying rather than merely noting: 51 of the family's 1,001 seed
records mention LK-99 or the lead apatite, and 41 of them fall in
2023Q3, which is 39\% of that quarter's 105 cuprate records. The cuprate
row of Table 1 is inflated by that much. The growth figure on the
cuprate panel of Figure 3a is not: the burst sits mid-window and misses
both endpoint windows, so the family's +19\% is unchanged when the 51
rows are removed. No other family is measurably affected. We report the
labels as the released classifier produced them rather than patching
them afterwards, because the agreement figures in Section 2 are scored
against that same label file; a reader who wants the corrected cuprate
series can drop those 51 rows. The episode is visible in Figure 3a as a
sharp cuprate spike in 2023Q3, and it resolved quickly: phase-pure
single crystals proved highly insulating and optically transparent with
no transition anomalies, ruling out superconductivity
\citep{puphal2023single}. The episode is also a good example of how
first-principles work enters a live controversy on both sides: Griffin's
density-functional calculations identified correlated isolated flat
bands at the Fermi level, noting these as a signature associated with
high transition temperatures \citep{griffin2023origin}, which made the
compound worth taking seriously even as the crystals were failing to
superconduct. The replication burst is the most compressed example in
this corpus of the field's self-correction working, and of how quickly a
...
and show the steepest decline in quarterly output over the window, at
-10\%; topological (-8\%) and other unconventional systems (-5\%) are
the only others that fall at all (Figure 3a). Fifteen years after the
founding report \citep{kamihara2008iron}, the work is consolidation:
...
This family (1,329 seed records) is the second-largest named material
family, after conventional systems, and the corpus's most
methodologically distinctive: superconductivity here is tuned
...
The largest single label in the corpus is not a material at all: 1,861
...
which is why neural quantum states occupy this mode (Section 5). They
are not the only AI work filed under it: of the 124 AI-tagged theory
records in the merged corpus of Figure 4, 40 are neural quantum states
and the remaining 84 are surrogate or regression models (67), graph
networks and machine-learned potentials (8), generative models (5) and
language-model or agent work (4). What distinguishes the
neural-quantum-state thread is not that it is alone in the mode but that
its output is checkable inside the mode, against other solvers on the
same Hamiltonian. The distinction matters for Section 6: theory is not
one cell for AI purposes but two, and only the computational half is
addressable today.
...
36-million-structure hydride exploration \citep{wang2025discoverya}. No
other mode in this survey has a comparable substrate. The cost is the
well-known one: the pipeline is trustworthy for phonon-mediated pairing
and does not transfer to the correlated families, which is exactly why
237 of the 530 ab initio records sit in the conventional family and only
54 in cuprates.
...
The corpus's own signature is stark. The systematic seed contains no
AI-tagged synthesis paper at all; the single one in the merged corpus
arrived through the targeted supplement, and it is a language agent
bringing up a 112-qubit processor rather than a materials synthesis.
Section 6 argues that the tractable target here is not automating
synthesis but predicting synthesizability.
...
preprints in the systematic seed, 120 use a machine-learning method in
their own work: 1.2\%. The count grows across the four complete calendar
years in the window -- 13 in 2022, 19 in 2023, 22 in 2024, 38 in 2025,
which is 92 of the 120; the remaining 28 fall in the two partial years,
September to December 2021 and January to September 2026 -- but from a
base so low that even a tripling leaves the field's daily practice
essentially untouched. A recent review of computational superconductor
discovery reaches the same conclusion from the opposite direction,
describing AI and machine-learning efforts in this field as remaining
``in its infant stage'' while the predictive work continues to be
carried by Migdal-Eliashberg theory and its first-principles
implementations \citep{tran2025superconductor}.
...
characterization, 16 ab initio, and 1 synthesis. The seed alone, which
is the part that can carry a rate, gives 55 discovery, 46 theory, 10
characterization, 9 ab initio and no synthesis; the supplement's keyword
threads are what pull theory to the top of the merged count. Either way,
machine learning in this field is something that happens to a
calculation or to a dataset, almost never to a furnace.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Figure 4: AI-method usage by mode of inquiry across the working corpus. Synthesis carries a single record, and it comes from the supplement: the systematic seed has none. The figure combines 120 seed records with 133 from the targeted supplement, so 53\% of it was retrieved by keyword rather than sampled systematically. It therefore describes the AI-for-superconductivity literature this survey assembled, not an adoption rate; the rate is the seed-only 1.2\% quoted above.}]{figures/fig4_ai_method_task.png}}
\caption*{Figure 4: AI-method usage by mode of inquiry across the working
corpus. Synthesis carries a single record, and it comes from the
supplement: the systematic seed has none. The figure combines 120 seed
records with 133 from the targeted supplement, so 53\% of it was
retrieved by keyword rather than sampled systematically. It therefore
describes the AI-for-superconductivity literature this survey assembled,
not an adoption rate; the rate is the seed-only 1.2\% quoted above.}
\end{figure}
...
the rest of this literature does not: four proposed compounds were
synthesised and measured -- Zr\(_3\)ScRe\(_8\) at \(T_c\) = 6.5 K,
HfZrRe\(_4\) at 5.9 K, Zr\(_4\)VRe\(_7\) at 3.5 K, and
Hf\(_{21}\)Re\(_{25}\) at 2.5 K. These transition temperatures are low,
and that is the point worth holding onto. The loop closed on compounds
an order of magnitude below the temperatures the screening papers
advertise.

It is not the only closed loop in the window. Four cycles of prediction,
experimental test and retraining more than doubled the success rate of
superconductor discovery, found a new superconductor in the Zr-In-Ni
system, and recovered five superconductors absent from the training data
\citep{pogue2022closed}. That work satisfies all three parts of this
survey's definition of an AI-found superconductor, and it does the one
thing the more recent case does not, which is feed the measurement back
into the model. We therefore do not claim these two are the only such
loops in the field: they are the ones this corpus surfaced, and no
exhaustive search for closed loops was attempted. What both share is the
regime they closed in -- low-temperature intermetallics at ambient
pressure, where the synthesis is routine.
...
synthesis. The A-Lab report is usually cited for a single number, and
the number needs care: 41 of 58 targets were realised autonomously over
17 days of operation, and 43 products in total once runs with human
intervention are included \citep{szymanski2023autonomous}. Taking that
work as their example, Leeman and co-workers examined all 43 synthetic
products, identified four recurring shortfalls in the analysis, and
concluded that no new materials had been discovered in it
\citep{leeman2024challenges}. A subsequent author correction narrowed
the novelty claim to materials that were ``new to the prediction
platform, not necessarily new to science'', reported that a
peer-reviewed re-analysis of the diffraction data upheld 36 of 40
re-examined successes with four inconclusive, and removed one compound,
Zn\(_2\)Cr\(_3\)FeO\(_8\), that had mistakenly been present in the
training data \citep{szymanski2026correction}. The three claims --
novelty, correct identification of the phases made, and autonomous
execution -- have to be judged separately, and the training-data leakage
is a small instance of the label problem this survey returns to below.
Neither critique says the programme is worthless. Both say the
validation step is where the difficulty lives, which is the same thing
this survey's corpus says about superconductivity specifically.
...
Machine learning in superconductivity is currently, in the main, a way
of making expensive calculations cheaper and instrument data more
usable. It has in a small number of documented cases also guided the
discovery of experimentally confirmed superconductors
\citep{pogue2022closed, li2026agentic}, and that remains the exception
rather than the mode. The evidence for that reading is the shape of the
corpus rather than any single paper: 1.2\% penetration in the systematic
seed, no AI-tagged synthesis paper in it, and closed loops that
delivered intermetallics at 2.5 to 6.5 K with no applied pressure
reported, while the screening literature advertises predictions above
200 K at 200 GPa. The two numbers describe different regimes, and the
point is not that one refutes the other: the loop closed where the
synthesis was routine and stays open where the headline predictions
live. The next section asks, cell by cell, what would have to change.
...
Section 5 described what has happened. This section asks what could.
Figure 5 scores three properties on a 0-3 scale and names the constraint
that currently binds, for 15 of the taxonomy's 45 cells: the ones the
preceding sections discussed at length. Iron-based systems and the other
unconventional families are not scored, and the unscored cells should
not be read as scoring zero -- they are cells this survey did not
examine closely enough to rate.

\emph{Data availability} asks whether enough machine-readable, labelled
data exists to train or evaluate a model. Benchmark maturity asks
whether a shared task with an agreed metric and a held-out set exists,
so that two methods can be compared without re-running each other's
pipelines. Cheap validation asks what it costs to find out whether a
prediction was right -- a score of 3 means a calculation settles it, a 0
means a hard experiment is required. The binding bottleneck names which
of data, compute, theory or experiment is the constraint that would have
to move first.

These scores are the author's judgement, not measurements, and they
should be read as a structured argument rather than as data. Their
unweighted sum is used to order the display; it is not an estimate of
scientific value or of expected return on effort. They are stated on a
coarse scale precisely because a finer one would imply a precision that
is not there. The corpus counts inform them but do not determine them: a
cell can be crowded and unready, and Section 8 argues that several are.

\begin{figure}
\centering
\pandocbounded{\includegraphics[keepaspectratio,alt={Figure 5: AI readiness for 15 of the taxonomy's 45 cells -- the ones Sections 3 and 4 discuss at length. Scores are the author's judgement on a 0-3 scale, not measurements; unscored cells are unrated, not zero-rated; the appendix states the reasoning for each scored cell.}]{figures/fig5_ai_readiness.png}}
\caption*{Figure 5: AI readiness for 15 of the taxonomy's 45 cells -- the
ones Sections 3 and 4 discuss at length. Scores are the author's
judgement on a 0-3 scale, not measurements; unscored cells are unrated,
not zero-rated; the appendix states the reasoning for each scored cell.}
\end{figure}
...
\emph{Cheap-validation cells, where progress is fastest and already
visible.} Conventional and hydride ab initio work scores highest: a
machine-readable training set exists -- about 7,000 electron-phonon
calculations assembled to screen roughly 200,000 compounds in the
largest such study \citep{cerqueira2023sampling} -- an accepted target
quantity exists in the Eliashberg \(T_c\), and a prediction can be
checked by running the calculation the model was trained to approximate.
Material-agnostic theory sits alongside it: a neural quantum state's
variational energy is checkable against other methods on the same
Hamiltonian. Both cells are compute-limited rather than data-limited,
and both are where the corpus already shows the most AI activity. This
is not a coincidence, and it is the single strongest regularity in the
survey: \emph{machine learning has penetrated exactly those cells where
a machine can grade its own homework.}

\emph{Data-limited cells, where the obstacle is that measurements are
...
data -- but in the papers read for this survey it reached the reader as
figures inside PDFs, in per-group formats, and in supplementary files
without schemas. As Section 4 says, we did not audit data-availability
practice across the field, and this scoring rests on that reading rather
than on a survey of release policies. The one recent large-scale attempt
to fix this for superconductivity extracted 78,203 records with a
language-model workflow \citep{itani2025large}, which is both an
...
\emph{Experiment-limited cells, where nothing else is the bottleneck.}
The conventional, nickelate and two-dimensional synthesis cells all
score 1, 0, 0. There is no shared dataset of attempted syntheses with
their outcomes, no benchmark, and validation requires making the
material. Device synthesis is the one exception, at 2, 1, 1, because
semiconductor-style fabrication generates process data even where little
of it is published. The corpus carries the corresponding signature: no
AI-tagged synthesis paper in the 10,049-record seed, and one in the
merged corpus of 10,249. This is where the general AI-for-materials
programme has invested most visibly, in autonomous laboratories
\citep{szymanski2023autonomous}, and where the criticism has been
sharpest \citep{leeman2024challenges}. Superconductivity makes the
problem harder than the oxide-powder case those systems target:
nickelate films require topotactic reduction, hydrides require diamond
anvil cells at hundreds of gigapascals, and moiré devices require
stacking with angular precision. None of these is a pipetting robot's
problem.
...
specific: for the prediction task these cells are usually asked to
perform, there is no agreed target quantity a model could learn to
regress, so more data of the same kind does not by itself define a
learnable objective. That is a statement about a task, not a claim that
no learning objective can be defined in these cells at all. Cuprate
theory is the canonical case. Four decades of data have not produced an
agreed mechanism, so there is no equivalent of the Eliashberg function
to regress against. Nickelate ab initio work has the opposite problem in
the same family: the calculations are tractable and increasingly
standard, but which calculated quantity predicts \(T_c\) in a
multi-orbital correlated system is exactly what is unsettled.
...
Ordering the 15 scored cells by summed score produces a priority list,
and it is not the list the field's activity implies. The highest-scoring
...
The ends of that ordering are more robust than its middle. Conventional
ab initio scores at least as high as every other cell on all three axes,
so it stays first under any non-negative weighting; the synthesis cells
stay last for the same reason. The three cells tied at seven change
places as soon as one axis is weighted above another, and none of this
tests the scores themselves, only what follows from them. Readiness in
this sense is also not the same as value: a cell can be ready and
unimportant, and the ordering says nothing about the cost of acting on
it.

Three consequences follow, and each is a claim that could be checked
rather than a prediction we are confident in.

\emph{Characterization inversion is the most under-exploited
...
\emph{Synthesis will not yield to autonomous laboratories in this field
on the current design.} The binding constraint is that
...
\emph{The field needs a held-out benchmark with experimental
...
The pattern across the scored cells is that AI adoption in this field
lines up with validation cost better than with data volume or scientific
importance. Where a calculation can check a prediction, methods have
arrived. Where an experiment is needed, they have not, regardless of how
much has been written about the cell. This is a correspondence between
two things the survey observes, not a controlled comparison: we did not
measure validation cost independently of adoption, and Section 8 states
it as a hypothesis with the evidence that would test it. The three
priorities that follow are stated above; each is a dataset or standards
problem rather than a modelling one, which is why none of them is
glamorous.
...
Section 6 identified experiment as the binding constraint in every
synthesis cell. That section did not isolate instrument integration as
the cause of that constraint, and nothing in this survey measures
integration cost separately from synthesis time, sample yield or the
cost of a failed run; the argument below is about a plausible lever on a
...
\emph{A synthesizability model trained on failures.} Section 4
...
here for a reason usually left implicit: a robot has no incentive to
withhold a failed run. Instrumented synthesis generates the negative
data that human publication practice systematically destroys.

\emph{Instrumented characterization as a first-class output.} The same
...
\emph{Extreme-condition autonomy, which is the hard case.}
...
measured in years. Hardware standardisation does not remove it either:
the Model Hardware Standard preview states that it does not yet work
with hardware lacking a programming interface \citep{anthropic2026mhs},
which is a fair description of most extreme-condition apparatus. We
suggest, as a judgement rather than a result, that the
diamond-anvil-cell case is the one worth attacking first: the sample is
tiny, the measurement is electrical, the parameter space is
low-dimensional, and the iteration appears to us to be gated largely by
human attention. We have not measured that, and a comparison of
candidate platforms would need documented integration effort, throughput
and failure rates.

\emph{A benchmark with experimental resolution.} The AI-for-science
...
against what is subsequently made and measured. A benchmark of that
shape tests one specific thing -- whether a method's ranking holds up on
materials confirmed after its predictions were fixed -- which
retrospective scoring on a fixed dataset cannot, and its absence is why
no cell in Figure 5 scores full marks on benchmark maturity.
Chronological separation is not the same as distributional separation: a
time-stamped benchmark shows that a prediction preceded its
confirmation, and a claim about extrapolating away from the training
distribution would need chemical or structural held-out splits as well.
...
closed prediction-synthesis-measurement loops this survey found
delivered low-temperature intermetallics -- 2.5 to 6.5 K with no applied
pressure reported in the more recent case \citep{li2026agentic}, a new
Zr-In-Ni compound in the earlier one \citep{pogue2022closed} --
compounds whose synthesis is routine, which is part of why the loop
could close at all. A faster loop that stays in that regime would be a
genuine engineering achievement and no progress toward the grail,
because the constraint at high \(T_c\) is not iteration speed but
extreme-condition synthesis and the absence of a theory that says where
to look in the correlated families.
...
not ``apply a generative model to composition space.'' It is the three
priorities of Section 6, with one step placed alongside them. The four
can be pursued in parallel; the dependencies below are about what makes
each easier, not about a required order:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \emph{Capture the experiments in structured form.} This step is new
  here, and it feeds the other three: the negative results and the
  machine-readable characterization records they consume have to come
  from somewhere, and nothing in current publication practice produces
  them. Automation is one way to capture them consistently and not the
  only one; manual and assisted workflows can do it too, given metadata
  standards, quality control and a release practice.
\item
  \emph{Build the benchmark}, because without time-stamped experimental
  resolution the screening literature cannot be compared and therefore
  cannot improve.
\item
  \emph{Model synthesizability}, because that is the tractable form of
  the bottleneck and it converts screening output into experimental
  input.
\item
  \emph{Attack the theory bottleneck where it is well-posed}, meaning
...
Third, labels come from a model pass with 86.3\% family and 87.0\% mode
agreement against a single annotator's hand labels, and the two smallest
modes -- synthesis at 7 of 11 and discovery at 5 of 13 -- are the least
reliable, on denominators small enough that two papers move the rate by
fifteen points. Any statement below that depends on those two counts is
...
\emph{The verifiability gradient.} AI methods appear in this corpus
...
question, has none in the seed. The hypothesis is that adoption is
governed by validation cost rather than by data volume or scientific
payoff. It is testable: if it holds, the next AI methods to arrive in
this field should show up in characterization inversion tasks, where an
independent measurement provides the check, before they show up anywhere
in synthesis.

\emph{The label-quality trap at the top of the range.} A \(T_c\) model
...
\emph{Category placement: AI-tagged papers are filed as
superconductivity papers.} We expected the opposite, and the corpus says
otherwise. If AI methods were arriving with machine-learning groups from
outside, AI-tagged preprints should be filed under different primary
categories from the rest of the corpus. They are not: 68\% of AI-tagged
seed papers carry \texttt{cond-mat.supr-con} as their primary category,
against 69\% of everything else. What does separate them is reach in the
other direction -- 16\% of AI-tagged papers cross-list to a
\texttt{cs.*} or \texttt{stat.*} category, against 0.3\% of the rest, a
fifty-fold difference.

These differences describe where authors file their papers. They do not
identify disciplinary background, prior expertise, or the direction in
which methods travel between communities: arXiv records carry no
affiliations, and an earlier draft of this hypothesis asserted the
affiliation claim directly, which nothing in this corpus supports. What
the category evidence does support is the weaker statement that
AI-tagged superconductivity work is being filed as superconductivity
work, and reaches outward to computer science rather than arriving from
it. Deciding whether methods are taken up internally or imported would
need author-level evidence this survey does not have.
...
The three priorities of Section 6 are also the three tests of it, and
each fails in a way that would be visible. If a public, standardised
corpus of labelled characterization data appeared and the data-limited
cells did not move toward method-limited, then format was not what was
blocking them. If a registry of attempted syntheses were assembled and
synthesizability models trained on it did not improve the hit rate of
the screening pipelines, then the missing negative results were not the
binding constraint. And if a time-stamped benchmark showed the current
screening methods ranking post-2024 experimental confirmations well,
then the label-quality problem this survey emphasises is smaller than
argued.

We would rather this section be checkable than comprehensive, so we
state the prediction it rests on plainly: the assessment in Figure 5 is
a claim about validation cost, and it is wrong if AI methods arrive in
the experiment-limited cells before the data and benchmark problems in
them are solved.
...
work that is never posted. It is also a category harvest, not a topic
harvest, and relevant work filed elsewhere is missed by construction:
the agentic screening study discussed in Section 5 is classified
\texttt{cs.LG} and \texttt{cond-mat.mtrl-sci} with no
\texttt{cond-mat.supr-con} listing \citep{li2026agentic}, and the
scanning-tunnelling study in the same section is filed under
\texttt{cond-mat.str-el} \citep{taranto2022unsupervised}. Both entered
through the targeted supplement rather than the seed, which is the
clearest available measure of what a single-category harvest costs. The
window opens in September 2021, so pre-window developments enter only
through hand-verified anchor citations. Labels are single-valued where
the underlying reality is often multi-valued, so a paper that
synthesises and measures a computationally proposed compound receives
one label, and the mode counts do not enumerate the experimental stages
inside a study. The AI lens has 88\% precision and 77\% recall on its
validation sample, so the detected rate is more likely to understate AI
use than to overstate it; because that sample was stratified on the
rule-based tag rather than drawn from the population, we do not report a
recall-corrected rate. The AI-readiness scores in Figure 5 are the
author's judgement on a coarse scale, informed by the corpus but not
derived from it, and the appendix states the reasoning for each scored
cell so that individual scores can be contested. All hand labels were
produced by one annotator, so the agreement figures quoted throughout
measure consistency between one human pass and one model pass, not
...
family and mode of inquiry, and then asked what a machine-learning
method could contribute in them.
...
bottleneck, synthesis, is the mode with the least usable data and the
fewest methods: no AI-tagged paper in the seed, and one in the merged
corpus.
...
report candidates above 200 K at 200 GPa; the closed
prediction-synthesis-measurement loops this survey found in the window
delivered low-temperature intermetallics, 2.5 to 6.5 K with no applied
pressure reported in the more recent case
\citep{li2026agentic, pogue2022closed}. The two sit in different
regimes, and that is the substance of the gap rather than a caveat to
it: the loops closed where the synthesis was easy. Both facts are real,
and holding them together is the correct posture toward AI for
superconductivity.

For anyone deciding where to apply AI for Science effort in this field,
the ordering that follows from the 15 cells scored in Figure 5 is:
release labelled characterization data and build inversion models
against it; construct a time-stamped benchmark so screening methods can
be compared; and model synthesizability rather than attempting to
automate synthesis. These are less exciting than inverse-designing a
room-temperature superconductor, and they are what the structure of the
field currently supports.

Section 7 argues that the first two of these are becoming easier to
build for reasons outside this field: laboratory hardware is being
standardised for machine operation, and an instrumented experiment emits
the negative results and the machine-readable spectra that human
publication practice does not. That makes the instrument interface a
candidate for the variable most likely to move this assessment, ahead of
model capability -- a prediction this survey advances rather than one it
measures. It also sets the test: if throughput rises while the AI-tagged
synthesis count stays near one, automation will have accelerated the
modes that were never the bottleneck.
...
\bottomrule\noalign{}
\endlastfoot
Conventional × ab initio & Data 3: \textasciitilde7,000 electron-phonon
calculations assembled as a single training set, used to screen roughly
200,000 compounds \citep{cerqueira2023sampling}. Benchmark 2: the
Eliashberg \(T_c\) is an accepted target, but no shared held-out set.
Validation 3: a DFPT calculation settles it. Bottleneck compute. \\
...
theory. \\
Cuprate × characterization & Data 2: abundant spectra, very little of it
encountered in machine-readable form. Benchmark 1, validation 2.
Bottleneck data. \\
