...
def main():
    ...
    # anchors.bib is hand-curated and web-verified, so where a corpus row resolves to
    # a key anchors.bib already defines, the anchor wins and we skip the corpus copy.
    # Emitting both produced a duplicate key, which BibTeX 0.99 (arXiv's) treats as a
    # fatal "Repeated entry" -- and meanwhile citeproc silently used whichever came
    # first, which for sun2023superconductivity was the arXiv title rather than the
    # verified published one.
    anchors = ROOT / "draft" / "anchors.bib"
    if anchors.exists() and not a.all:
        anchor_keys = set(re.findall(r"@\w+\{([^,]+),", anchors.read_text()))
        shadowed = [k for k, _ in chosen if k in anchor_keys]
        if shadowed:
            print(f"skipping {len(shadowed)} key(s) already defined in anchors.bib:",
                  ", ".join(sorted(shadowed)))
        chosen = [(k, r) for k, r in chosen if k not in anchor_keys]
    cache = load_xref()
